...
**/
	tBTNodePtr prev = NULL, act = (*RootPtr);
	while (act) {
		prev = act;
		if (act->Cont < Content) {
			act = act->RPtr;
			...
			act = act->LPtr;
		} else if (act->Cont == Content) { // přesunout nahoru kvůli vyhodnocení
			return;
		}
	}
